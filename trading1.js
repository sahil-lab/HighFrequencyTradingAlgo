...
async function checkPrices() {
  try {
    ...
    logger.debug(`Received dynamicBaseProbability: ${dynamicBaseProbability.toFixed(2)}%`);
    ...
    if (probability.greaterThanOrEqualTo(MIN_PROBABILITY) && probability.lessThanOrEqualTo(MAX_PROBABILITY)) {
      ...
      if (TRADE_TYPE === 'spot') {
        logger.info(`${capitalizeFirstLetter(TRADE_MODE)} Spot trade with amount: ${amount.toFixed(2)} AVAX.`);
...
async function placeSpotTrade(entryPrice, amount, type, direction = 'long') {
  try {
    ...
    logger.info(
      `Placing ${type} Spot trade with amount: ${amount.toFixed(2)} AVAX at price: ${entryPrice.toFixed(2)} USDT.`
    );
    logger.info(
...
async function placeFutureTrade(entryPrice, amount, direction, type, allocation = 'favorable') {
  try {
    ...
    logger.info(
      `Placing ${type} ${direction} trade with amount: ${amount.toFixed(2)} AVAX at price: ${entryPrice.toFixed(
        2
      )} USDT.`
    );
    logger.info(
