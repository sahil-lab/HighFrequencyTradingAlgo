...
const logger = require('./logger-old'); // Ensure this is updated to use a robust logger like Winston
const {
  calculateRSI,
  calculateMACD,
  calculateSMA,
  ...
  try {
    ...
    if (type === 'real') {
      if (type === 'futures') {
        ...
        realBalance = realBalance.minus(entryPrice.times(amount).plus(entryFee));
        logger.debug(`Real balance after trade: ${realBalance.toFixed(2)} AVAX`);
      }
    } else {
      if (type === 'futures') {
        ...
        fakeBalance = fakeBalance.minus(entryPrice.times(amount).plus(entryFee));
        logger.debug(`Fake balance after trade: ${fakeBalance.toFixed(2)} AVAX`);
      }
    }
