// trading.js - Enhanced Trading Logic with Detailed Logging and Weighted Base Probability

require('dotenv').config();

const mongoose = require('mongoose');
const binance = require('./config');
const logger = require('./logger-old'); // Updated to use winston logger
const {
  calculateRSI,
  calculateMACD,
  calculateSMA,
    ...
    const balances = await limiter.schedule(() =>
      new Promise((resolve, reject) => {
        binance.balance((error, balances) => {
          if (error) {
            logger.error(`Failed to fetch balance using Binance library: ${error.body}`);
            return reject(error);
          }
          resolve(balances);
        });
    ...
    const ticks = await limiter.schedule(() =>
      new Promise((resolve, reject) => {
        binance.candlesticks('AVAXUSDT', interval, (error, ticks) => {
          if (error) {
            logger.error(`Failed to fetch OHLC data: ${error.body}`);
            return reject(error);
          }
          resolve(ticks);
        }, { limit });
    ...
    const fees = await limiter.schedule(() =>
      new Promise((resolve, reject) => {
        binance.tradeFee({ symbol }, (error, fees) => {
          if (error) {
            logger.error(`Failed to fetch spot trading fee: ${error.body}`);
            return reject(error);
          }
          resolve(fees);
        });
...
function displayTradeStatus(currentPrice) {
  logger.info(`Current Price: ${currentPrice} USDT`);
}
...
function validateHistoricalData(data) {
  for (let i = 1; i < data.length; i++) {
    ...
    if (data[i].openTime !== expectedOpenTime) {
      logger.warn(`Data gap detected between ${new Date(data[i - 1].closeTime).toISOString()} and ${new Date(data[i].openTime).toISOString()}.`);
    }
  }
}
...
async function monitorActiveTrades() {
  for (const trade of activeTrades) {
    try {
      ...
      logger.error(`Error monitoring trade: ${error.message}`);
    }
  }
}

async function calculateBaseProbability(windowSize = 100) {
  try {
    ...
    const lambda = 0.1; // Decay factor (tune as needed)
    let weightedSum = new Decimal(0);
    let weightTotal = new Decimal(0);

    recentTrades.forEach((trade, index) => {
      const weight = new Decimal(Math.exp(-lambda * index));
    ...
    const weightedWinRate = weightTotal.isZero() ? 0 : weightedSum.dividedBy(weightTotal).times(100);
    logger.info(`Calculated weighted base probability: ${weightedWinRate.toFixed(2)}% based on ${totalRecentTrades} recent trades.`);
    return new Decimal(weightedWinRate);
  } catch (error) {
...
async function checkPrices() {
  try {
    ...
    if (ohlc.length < 14) {
      logger.warn('Not enough historical data to calculate indicators. Skipping this cycle.');
      return;
    }
    ...
    if (timeElapsed >= 900) { // 15 minutes
      ...
      cumulativeProbabilityChange += Decimal.abs(probability.minus(dynamicBaseProbability)).toNumber();
      logger.debug(`Cumulative probability change: ${cumulativeProbabilityChange}`);
    }
    ...
    const order = await limiter.schedule(() =>
      new Promise((resolve, reject) => {
        binance.buy('AVAXUSDT', quantity, null, { type: orderType }, (error, response) => {
          if (error) {
            logger.error(`Failed to place Spot order: ${error.body}`);
            return reject(error);
          }
          resolve(response);
        });
    ...
    const order = await limiter.schedule(() =>
      new Promise((resolve, reject) => {
        binance.futuresOrder(orderSide, 'AVAXUSDT', quantity, null, { type: orderType }, (error, response) => {
          if (error) {
            logger.error(`Failed to place Futures order: ${error.body}`);
            return reject(error);
          }
          resolve(response);
        });
