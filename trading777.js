...
db.on('error', (error) => logMessage(`MongoDB connection error: ${error.message}`));
db.once('open', () => logMessage('Connected to MongoDB successfully.'));

// Define Mongoose Schema for OHLC Data
...
// Utility function to fetch candlestick data with dynamic symbol and interval
async function fetchCandlesticks(symbol = 'AVAXUSDT', interval = '15m', limit = 1000) {
  return new Promise((resolve, reject) => {
    binance.candlesticks(symbol, interval, (error, ticks) => {
      if (error) {
        logMessage(`Failed to fetch candlesticks for ${symbol} at ${interval}: ${error.body}`);
        reject(error);
      } else {
        resolve(ticks);
      }
...
async function getAvaxPrice(symbol = 'AVAXUSDT', interval = '1m') {
  try {
    const ohlc = await fetchCandlesticks(symbol, interval, 1); // Fetch the latest candlestick
    const latestCandle = ohlc[ohlc.length - 1];
    logMessage(`Current ${symbol} Price: ${latestCandle.close}`);
...
// Function to Fetch Comprehensive Historical Data from MongoDB
async function fetchHistoricalDataFromDB(symbol, interval, startTime, endTime) {
  try {
    ...
    logMessage(`Error fetching historical data from DB: ${error.message}`);
    throw error;
  }
}

// Function to Save Historical Data to MongoDB
async function saveHistoricalDataToDB(symbol, interval, candles) {
  try {
    ...
    // Bulk Insert or Update to Prevent Duplicates
    ...
    logMessage(`Saved ${ohlc.length} candles to MongoDB.`);
  } catch (error) {
    logMessage(`Error saving historical data to DB: ${error.message}`);
  }
}
...
async function initializeData(symbol, interval, historicalStartTime) {
  ...
  if (fetchedHistoricalData.length === 0) {
    logMessage('No historical data found in DB. Fetching from Binance API.');
    // Fetch from Binance API if DB has no data
    fetchedHistoricalData = await fetchCandlesticks(symbol, interval, 1000); // Fetch max limit
    await saveHistoricalDataToDB(symbol, interval, fetchedHistoricalData);
  } else {
    logMessage(`Fetched ${fetchedHistoricalData.length} historical data points from DB.`);
    // Fetch latest data from Binance API to ensure data is up-to-date
    const latestCandleTime = fetchedHistoricalData[fetchedHistoricalData.length - 1].closeTime;
    const newCandles = await fetchCandlesticks(symbol, interval, 1000); // Fetch latest 1000 candles
    // Filter new candles that are not in DB
    ...
    if (filteredNewCandles.length > 0) {
      ...
      fetchedHistoricalData = fetchedHistoricalData.concat(filteredNewCandles.map(candle => ({
      ...
      logMessage(`Added ${filteredNewCandles.length} new candles to historical data.`);
    } else {
      logMessage('No new candles to add.');
    }
  }
  ...
  ws.on('message', async (data) => {
    ...
    if (kline.x) { // If the candle is closed
      ...
      try {
        ...
        logMessage('Live candle saved to MongoDB.');
      } catch (error) {
        logMessage(`Error saving live candle to DB: ${error.message}`);
      }
    }
  });
...
// Function to Check Prices and Initiate Trades
async function checkPrices() {
  ...
  try {
    ...
    // Extract necessary price data from historicalData
    const closePrices = historicalData.map(bar => bar.close);
    const highPrices = historicalData.map(bar => bar.high);
    const lowPrices = historicalData.map(bar => bar.low);

    // Technical Indicators
    const rsi = calculateRSI(closePrices, 14);
    const macd = calculateMACD(closePrices, 12, 26, 9);
    const sma = calculateSMA(closePrices, 14);
    const ema = calculateEMA(closePrices, 14);
    const stochastic = calculateStochastic(highPrices, lowPrices, closePrices, 14, 3);
                ...
                rl.question(`Enter amount to use for ${tradeMode} ${tradeType} trading (Available: ${(tradeMode === 'real' ? realBalance : fakeBalance)} AVAX): `, async (amount) => {
                  ...
                  if (amount > balance) {
                    ...
                    if (tradeType === 'spot') {
                      ...
                      await placeSpotTrade(getLastClosePrice(), amount, tradeMode); // Use latest close price
                    } else {
                      ...
                      await placeFavorableTrade(getLastClosePrice(), favorableAmount, 'long', tradeMode, 'favorable');
                      await placeUnfavorableTrade(getLastClosePrice(), unfavorableAmount, 'short', tradeMode, 'unfavorable');
                    }
                    activeTrade = true;
                  }
                });
...
function calculateProbability(rsi, macd, sma, ema, stochastic, atr, bollinger) {
  ...
  if (bollinger.close < bollinger.lower) {
    probability += 5;
  } else if (bollinger.close > bollinger.upper) {
    probability -= 5;
  }
...
// Deprecated: Use placeFavorableTrade and placeUnfavorableTrade instead
async function placeTrade(entryPrice, amount, type) {
  logMessage('placeTrade function is deprecated. Use placeFavorableTrade and placeUnfavorableTrade instead.');
}
