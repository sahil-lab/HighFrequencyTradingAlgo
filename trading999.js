// trading.js - Enhanced Trading Logic with Dynamic Base Probability Based on Historical Trade Data
...
async function calculateBaseProbability(windowSize = 100) {
  try {
    ...
    if (totalRecentTrades === 0) return defaultBaseProbability; // Default if no data

    const recentWinningTrades = recentTrades.filter(trade => trade.outcome === 'win').length;
    const winRate = (recentWinningTrades / totalRecentTrades) * 100;

    return new Decimal(winRate);
  } catch (error) {
    logMessage(`Error calculating base probability: ${error.message}`, 'error');
