...
// Add this near the top of your file, after the logger import
// Add this logger initialization with fallback
global.logger = (() => {
  try {
    ...
    console.error('Logger initialization failed, using fallback logger');
    return {
      error: console.error,
      info: console.log,
      warn: console.warn,
...
// Updated to use the enhanced logger
...
const Decimal = require('decimal.js'); // Ensure this is imported
...
function safeToFixed(value, decimals) {
  return value && typeof value.toFixed === 'function' ? value.toFixed(decimals) : 'undefined';
}
...
async function getFuturesTradingFee(symbol) {
  try {
    const feeData = await retryOperation(() => limiter.schedule(() => binance.futuresCommissionRate(symbol)));
...
async function initializeData(symbol, interval, historicalStartTime) {
  ...
  logger.info(
    `Fetching historical data from ${new Date(historicalStartTime).toISOString()} to ${new Date(endTime).toISOString()}`
  );
...
async function calculateBaseProbability(windowSize = 100, alpha = 0.1) {
  ...
  try {
    ...
    recentTrades.forEach((trade, index) => {
      // Ensure trade.outcome is a string
      const outcome = trade.outcome ? trade.outcome.toString().toLowerCase() : 'undefined';
      ...
      logger.debug(`Trade ID: ${trade.id}, Outcome: ${outcome}, Weight: ${weight.toFixed(4)}`);
    });

    logger.debug(`Weighted Sum: ${weightedSum.toFixed(4)}, Weight Total: ${weightTotal.toFixed(4)}`);
    ...
    // Ensure weightedWinRate is a valid Decimal instance
    if (!(weightedWinRate instanceof Decimal)) {
      logger.error('weightedWinRate is not a valid Decimal instance. Falling back to default.');
      return defaultBaseProbability;
    }
  ...
  try {
    ...
    ohlc.forEach((candle, index) => {
      if (
        ...
        global.logger.error(`Candle at index ${index} is missing required fields.`);
        dataValid = false;
      }
    });

    if (!dataValid) {
      global.logger.error('Invalid OHLC data. Skipping this cycle.');
      return;
    }

    global.logger.debug('Calculating technical indicators.');
    ...
    global.logger.debug('Technical indicators calculated.');

    // Check if indicators are undefined
    if (
      rsi === undefined ||
      macd === undefined ||
      sma === undefined ||
      ...
      global.logger.error('One or more indicators returned undefined. Skipping this cycle.');
      return;
    }

    const dynamicBaseProbability = await calculateBaseProbability(); // Get from historical data

    if (!dynamicBaseProbability || typeof dynamicBaseProbability.toFixed !== 'function') {
      throw new Error(`dynamicBaseProbability is invalid: ${dynamicBaseProbability}`);
    }

    global.logger.debug(`Received dynamicBaseProbability: ${dynamicBaseProbability.toFixed(2)}%`);
    global.logger.debug(`Dynamic base probability: ${dynamicBaseProbability.toFixed(2)}%`);
    ...
    global.logger.debug(`Probability after indicators: ${probability.toFixed(2)}%`);
    ...
    if (timeElapsed >= 900) {
      ...
      startTime = Date.now();
      global.logger.debug('Resetting cumulative probability change after 15 minutes.');
    } else {
      cumulativeProbabilityChange = cumulativeProbabilityChange.plus(probability.minus(dynamicBaseProbability).abs());
      global.logger.debug(`Cumulative probability change: ${cumulativeProbabilityChange.toFixed(2)}`);
    }

    if (cumulativeProbabilityChange.greaterThan(20)) {
      probability = probability.minus(new Decimal(5));
      global.logger.warn('Cumulative probability change exceeded 20. Reducing probability by 5%.');
    }

    global.logger.info(`Adjusted Probability of Success: ${probability.toFixed(2)}%`);

    if (probability.greaterThanOrEqualTo(MIN_PROBABILITY) && probability.lessThanOrEqualTo(MAX_PROBABILITY)) {
      global.logger.info('Probability within favorable range. Proceeding to place trade.');

      timeframe = TRADE_TIMEFRAME;
      startPriceCheckInterval();

      const balance = TRADE_MODE === 'real' ? realBalance : fakeBalance;
      const amount = TRADE_AMOUNT_PERCENTAGE.times(balance).toDecimalPlaces(8);

      if (TRADE_TYPE === 'spot') {
        localLogger.info(`${capitalizeFirstLetter(TRADE_MODE)} Spot trade with amount: ${amount.toFixed(2)} AVAX.`);
        ...
        global.logger.info(
          `${capitalizeFirstLetter(TRADE_MODE)} Futures trade with favorable amount of ${favorableAmount.toFixed(
            2
      ...
      global.logger.info('Probability not in favorable range. Trade skipped.');
    }
  } catch (error) {
    global.logger.error(`Error checking prices: ${error.message}`);
    return;
  }
}
...
function calculateProbability(rsi, macd, sma, ema, stochastic, atr, bollinger, baseProbability) {
  ...
  // Ensure baseProbability is a Decimal instance
  if (!(probability instanceof Decimal)) {
    probability = new Decimal(probability || 0);
  }

  // Check if any inputs are undefined or invalid
  if (!rsi || !(rsi instanceof Decimal)) {
    logger.error('RSI is undefined or invalid.');
    return probability;
  }
  if (!macd || !macd.histogram || !(macd.histogram instanceof Decimal)) {
    logger.error('MACD histogram is undefined or invalid.');
    return probability;
  }
  if (!sma || !(sma instanceof Decimal)) {
    logger.error('SMA is undefined or invalid.');
    return probability;
  }
  if (!ema || !(ema instanceof Decimal)) {
    logger.error('EMA is undefined or invalid.');
    return probability;
  }
  if (!stochastic || !stochastic.k || !(stochastic.k instanceof Decimal) || !stochastic.d || !(stochastic.d instanceof Decimal)) {
    logger.error('Stochastic K or D is undefined or invalid.');
    return probability;
  }
  if (!atr || !(atr instanceof Decimal)) {
    logger.error('ATR is undefined or invalid.');
    return probability;
  }
  if (
    !bollinger ||
    !bollinger.price ||
    !(bollinger.price instanceof Decimal) ||
    !bollinger.lower ||
    !(bollinger.lower instanceof Decimal) ||
    !bollinger.upper ||
    !(bollinger.upper instanceof Decimal)
  ) {
    logger.error('Bollinger Bands values are undefined or invalid.');
    return probability;
  }

  // Detailed logging of indicator values
  logger.debug(`RSI: ${safeToFixed(rsi, 2)}`);
  logger.debug(`MACD Histogram: ${safeToFixed(macd.histogram, 2)}`);
  logger.debug(`SMA: ${safeToFixed(sma, 2)}, EMA: ${safeToFixed(ema, 2)}`);
  logger.debug(`Stochastic K: ${safeToFixed(stochastic.k, 2)}, Stochastic D: ${safeToFixed(stochastic.d, 2)}`);
  logger.debug(
    `Bollinger Bands - Price: ${safeToFixed(bollinger.price, 2)}, Lower: ${safeToFixed(
      bollinger.lower,
      2
    )}, Upper: ${safeToFixed(bollinger.upper, 2)}`
  );
  logger.debug(`Base Probability: ${safeToFixed(probability, 2)}%`);

  // Begin probability adjustments based on indicators

  // RSI adjustments
  if (rsi.lessThan(new Decimal(30))) {
    probability = probability.plus(new Decimal(10));
    logger.debug('RSI < 30: Increasing probability by 10%.');
  } else if (rsi.greaterThan(new Decimal(70))) {
    probability = probability.minus(new Decimal(10));
    logger.debug('RSI > 70: Decreasing probability by 10%.');
  }

  // MACD adjustments
  if (macd.histogram.greaterThan(new Decimal(0))) {
    probability = probability.plus(new Decimal(5));
    logger.debug('MACD Histogram > 0: Increasing probability by 5%.');
  } else if (macd.histogram.lessThan(new Decimal(0))) {
    probability = probability.minus(new Decimal(5));
    logger.debug('MACD Histogram < 0: Decreasing probability by 5%.');
  }

  // SMA and EMA adjustments
  ...
  if (stochastic.k.lessThan(new Decimal(20)) && stochastic.d.lessThan(new Decimal(20))) {
    probability = probability.plus(new Decimal(10));
    logger.debug('Stochastic K and D < 20: Increasing probability by 10%.');
  } else if (stochastic.k.greaterThan(new Decimal(80)) && stochastic.d.greaterThan(new Decimal(80))) {
    probability = probability.minus(new Decimal(10));
  ...
  if (probability.lessThan(new Decimal(0))) {
    probability = new Decimal(0);
    logger.debug('Probability clamped to 0%.');
  }
  if (probability.greaterThan(new Decimal(100))) {
...
async function placeSpotTrade(entryPrice, amount, type, direction = 'long') {
  try {
    ...
    if (type === 'real') {
      ...
      logger.debug(`Real balance after trade: ${safeToFixed(realBalance, 2)} AVAX`);
    } else {
      fakeBalance = fakeBalance.minus(totalCost);
      logger.debug(`Fake balance after trade: ${safeToFixed(fakeBalance, 2)} AVAX`);
    }
    walletBalance = walletBalance.minus(amount);
    logger.debug(`Wallet balance after trade: ${safeToFixed(walletBalance, 2)} AVAX`);
    ...
    logger.info(
      `Placing ${type} Spot trade with amount: ${safeToFixed(amount, 2)} AVAX at price: ${safeToFixed(
        entryPrice,
        2
      )} USDT.`
    );
    logger.info(
      `${capitalizeFirstLetter(type)} trade placed with stop loss at ${safeToFixed(
        stopLoss,
        2
      )} USDT and take profit at ${safeToFixed(takeProfit, 2)} USDT.`
    );
...
async function placeFutureTrade(entryPrice, amount, direction, type, allocation = 'favorable') {
  try {
    const feeRates =
      type === 'real' ? await getFuturesTradingFee('AVAXUSDT') : { takerFee: new Decimal(0), makerFee: new Decimal(0) };
    const feeRate = feeRates.takerFee;
    const entryFee = entryPrice.times(amount).times(feeRate);

    const initialMargin = type === 'real' ? entryPrice.times(amount).dividedBy(leverage) : new Decimal(0);

    const balance = type === 'real' ? realBalance : fakeBalance;
    const totalCost = type === 'real' ? initialMargin.plus(entryFee) : entryPrice.times(amount).plus(entryFee);
    ...
    logger.info(
      `Placing ${type} ${direction} trade with amount: ${safeToFixed(amount, 2)} AVAX at price: ${safeToFixed(
        entryPrice,
        2
      )} USDT.`
    );
...
async function handleTradeClosure(trade, currentPrice) {
  try {
    ...
    if (netPnl.isNegative() && isUnfavorable && !trade.isReallocated) {
      ...
      logger.warn(
        `Unfavorable trade ${trade.id} closed with loss. Reallocating ${safeToFixed(
          reallocateAmount,
          2
      ...
      await placeFutureTrade(currentPrice, reallocateAmount, newDirection, trade.tradeMode, 'favorable');

      trade.isReallocated = true;
    }

    logger.info(
      `${capitalizeFirstLetter(trade.tradeMode)} ${trade.direction.toUpperCase()} Trade ${trade.id
      } closed with ${netPnl.greaterThanOrEqualTo(0) ? 'profit' : 'loss'}: ${safeToFixed(netPnl, 2)} USDT.`
    );
    ...
    if (trade.tradeMode === 'fake') {
      fakeBalance = fakeBalance.plus(netPnl).plus(trade.amount);
      fakePnL = fakePnL.plus(netPnl);
      logger.debug(
        `Fake balance updated to ${safeToFixed(fakeBalance, 2)} AVAX and fakePnL to ${safeToFixed(fakePnL, 2)} USDT.`
      );
    } else if (trade.tradeMode === 'real') {
      realBalance = realBalance.plus(netPnl).plus(trade.amount);
      realPnL = realPnL.plus(netPnl);
      logger.debug(
        `Real balance updated to ${safeToFixed(realBalance, 2)} AVAX and realPnL to ${safeToFixed(realPnL, 2)} USDT.`
      );
    } else {
      spotPnL = spotPnL.plus(netPnl);
      walletBalance = walletBalance.plus(trade.amount);
      if (trade.tradeMode === 'real') {
        realBalance = realBalance.plus(trade.entryPrice.times(trade.amount));
        logger.debug(`Real balance updated to ${safeToFixed(realBalance, 2)} AVAX.`);
      } else {
        fakeBalance = fakeBalance.plus(trade.entryPrice.times(trade.amount));
        logger.debug(`Fake balance updated to ${safeToFixed(fakeBalance, 2)} AVAX.`);
      }
      logger.debug(
        `SpotPnL updated to ${safeToFixed(spotPnL, 2)} USDT and walletBalance to ${safeToFixed(walletBalance, 2)} AVAX.`
      );
    }
...
function finalizeTrade() {
  ...
  if (AUTO_TRADE_ENABLED) {
    global.logger.info('Auto trade enabled. Checking for next trade opportunity.');
    checkPrices().catch((error) => {
      global.logger.error(`Error initiating new trade: ${error.message}`);
    });
  } else {
    global.logger.info('Auto trade disabled. Trade session ended.');
    gracefulShutdown();
  }
}
