...
// Fetch OHLC Data for a Given Timeframe from Binance and Save to MongoDB
async function getAvaxOHLC(interval = '1m', limit = 60) { // Fetch last 60 periods
  try {
    ...
    logMessage(`Error fetching OHLC data: ${error.message}`);
    throw error;
  }
}
...
async function initializeData(symbol, interval, historicalStartTime) {
  ...
  if (fetchedHistoricalData.length === 0) {
    ...
    fetchedHistoricalData = await getAvaxOHLC(interval, 1000); // Fetch max limit
    ...
    const newCandles = await getAvaxOHLC(interval, 1000); // Fetch latest 1000 candles
