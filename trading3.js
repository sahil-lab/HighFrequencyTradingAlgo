// trading.js - Enhanced Trading Logic with Dynamic Timeframe Selection and Automated Reallocation
...
async function checkPrices() {
  ...
  try {
    ...
    const ohlc = await getAvaxOHLC(timeframe, 14); // Example: 14-period indicators
    ...
    if (probability >= 70 && probability <= 80) {
      rl.question('Do you want to take this trade? (yes/no): ', (answer) => {
        if (answer.toLowerCase() === 'yes') {
          ...
          rl.question('Select timeframe (1m, 3m, 5m, 15m, 30m, 1h): ', (selectedTimeframe) => {
